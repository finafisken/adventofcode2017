...
let loopStartIdx;
 ...
 do {
  ...
  if(previous.some(prevRes => arraysAreEqual(prevRes, memBanks))) {
    loopStartIdx = previous.findIndex(arr => arraysAreEqual(arr, memBanks));
  }
} while (!previous.some(prevRes => arraysAreEqual(prevRes, memBanks)))


console.log('loop length: ', previous.length - loopStartIdx);
